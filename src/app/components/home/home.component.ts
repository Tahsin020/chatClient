...
export class HomeComponent {
  ...
  constructor(private http: HttpClient) {
    ...
    this.hub = new signalR.HubConnectionBuilder()
      .withUrl('https://localhost:7138/chat-hub')
      .build();
    ...
    this.hub.start().then(() => {
      ...
      this.hub?.invoke('Messages', (res: ChatModel) => {
        console.log(res);
  ...
  getUsers() {
    this.http
      .get<UserModel[]>('https://localhost:7138/api/Chats/GetUsers')
      .subscribe((res) => {
  ...
  changeUser(user: UserModel) {
    ...
    this.http
      .get(
        `https://localhost:7138/api/Chats/GetChats?UserId=${this.user.id}&toUserId=${this.selectedUserId}`
      )
      .subscribe((res: any) => {
        this.chats = res;
      });
  ...
  sendMessage() {
    ...
    this.http
      .post<ChatModel>('https://localhost:7138/api/Chats/SendMessage', data)
      .subscribe((res) => {
        this.chats.push(res);
        this.message = '';
      });
